...
public:
	...
	void updateWithVelocity(CCPoint vel, float dt, float scale);
	void updateWithYPosition(float y, CCTime dt);

	void addInfiniteScrollWithZ(int z, CCPoint ratio, CCPoint pos, CCPoint dir,
			CCSprite *firstObject, ...);
	void addInfiniteScrollXWithZ(int z, CCPoint ratio, CCPoint pos,
			CCSprite* firstObject, ...);
	void addInfiniteScrollYWithZ(int z, CCPoint ratio, CCPoint pos,
			CCSprite* firstObject, ...);

	void addInfiniteScrollWithObjects(CCArray *objects, int z, CCPoint ratio,
			CCPoint pos, CCPoint dir);
	void addInfiniteScrollWithObjects(CCArray *objects, int z, CCPoint ratio,
			CCPoint pos, CCPoint dir, CCPoint relVel);
	void addInfiniteScrollWithObjects(CCArray *objects, int z, CCPoint ratio,
			CCPoint pos, CCPoint dir, CCPoint relVel, CCPoint padding);
